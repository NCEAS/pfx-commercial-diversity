...
# set user <- "your name"
user <- "Sean"
source(here::here("plots/plot-preamble.R"))
```
...
```{r}
length(ids_pre_1999$p_holder)
length(ids_post_1999$p_holder)
length(ids_pre_1999$p_holder[!ids_pre_1999$p_holder %in% ids_post_1999$p_holder])
```
...
ids_stuck_arround <-
  ids_pre_1999_only_herring$p_holder[ids_pre_1999_only_herring$p_holder %in%
    ids_post_1999$p_holder]

length(ids_pre_1999_only_herring$p_holder)
length(ids_stuck_arround)
```

```{r}
filter(d, p_holder %in% ids_pre_1999_only_herring$p_holder) %>%
  group_by(year) %>%
...
```{r}
wide <- reshape2::dcast(summary, year + p_holder + case_study ~ p_fshy)
wide$p_holder <- as.numeric(as.factor(wide$p_holder))
knitr::kable(wide)
...
out <- plyr::laply(seeds, function(x) {
  set.seed(x)
  kmeans(aqw[, -c(1:3), drop = FALSE], centers = 5, iter.max = 100)$tot.withinss
}, .parallel = TRUE)

# set the seed to one of the global minimums:
set.seed(seeds[[which(out == min(out))[[1]]]])
clust <- kmeans(aqw[, -c(1:3), drop = FALSE], centers = 5, iter.max = 100)
...
fig <- ggplot(rename(plot_dat, Fisheries = group), aes(year, n / n_year)) +
  geom_col(aes(fill = Fisheries), position = "stack", colour = "grey90",
    lwd = 0.3) +
  xlab("Year") +
  ...
  geom_vline(xintercept = 1999, lty = 2)

fig

ggsave(here::here("Fig_pws_herring2.pdf"), width = 4.8, height = 3)
```
